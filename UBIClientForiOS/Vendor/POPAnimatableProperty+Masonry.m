...
+ (POPAnimatableProperty *)mas_centerXOffsetProperty {
    return [POPAnimatableProperty propertyWithName:@"centerXOffset" initializer:^(POPMutableAnimatableProperty *prop) {
        prop.readBlock = ^(MASConstraint *constraint, CGFloat values[]) {
            values[0] = getLayoutConstant(constraint);
        };
