...
//TODO 扩展NSURLRequest熟悉，在if (_request.URL.isFileURL) { 代码块中配置选项
...
- (instancetype)initWithURL:(NSURL *)url
             customeRequest:(EVOWebImageCustomeRequest)request
                    options:(YYWebImageOptions)options
    ...
    if (!request) return nil;
    _url = url;
    _customeRequest = request;
    _options = options;
    _cache = cache;
    _cacheKey = cacheKey ? cacheKey : url.absoluteString;
    _shouldUseCredentialStorage = YES;
    _progress = progress;
    _transform = transform;
    _completion = completion;
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, [self _getRequestUrl], YYWebImageFromNone, YYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, [self _getRequestUrl], YYWebImageFromDiskCache, YYWebImageStageFinished, nil);
                [self _finish];
            } else {
        ...
        if (![self isCancelled]) {
            ...
            if (!image) {
                ...
                if (_options & YYWebImageOptionIgnoreFailedURL) {
                    if (URLBlackListContains([self _getRequestUrl])) {
                        ...
                        URLInBlackListAdd([self _getRequestUrl]);
                    }
                }
            }
            if (_completion) _completion(image, [self _getRequestUrl], YYWebImageFromRemote, YYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (NSURL *)_getRequestUrl {
    return _request?_request.URL : self.url;
}
...
- (void)start {
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if(_customeRequest){
                self.executing = YES;
                [self performSelector:@selector(_startCustomeRequest)
